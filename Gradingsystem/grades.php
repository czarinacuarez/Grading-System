                    ...
                    <div class="card">
                        ...
                        <?php
                                    ...
                                    while ($aa = $res->fetch_object()) {
                                        ...
                                        $sql = "SELECT AVG(grades.total_grade) AS average_value
                                        FROM groups
                                        INNER JOIN grades ON grades.group_id = groups.group_id
                        ...
                        <!-- Chart wrapper -->
                            ...
                            <?php
                            $sql = "SELECT CONCAT(user.firstname, ' ', user.lastname) AS name, grades.total_grade FROM grades INNER JOIN user ON user.id = grades.user_id
                            WHERE grades.group_id = '$id'";
                            $result = mysqli_query($conn, $sql);
                            ...
                            $sql_count = "SELECT COUNT(*) as total_count FROM grades  INNER JOIN user ON user.id = grades.user_id
                            WHERE grades.group_id = '$id'";
                            ...
                            $item['total_hrade'] = $item['total_grade'] / $total_count;
                            }
                            unset($item);
                            ?>
                        </div>
        ...
        <div class = "margin">
            ...
            <div class="table-responsive">
                            ...
                            <table class="table align-items-center table-flush">
                                ...
                                <tbody>
                                  <?php
                                    $ret = "SELECT * FROM  grades INNER JOIN user ON grades.user_id = user.id WHERE group_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {
                                    ?>
                                        <tr>
                                            ...
                                            <td><?php echo $aa->integration ?></td>
                                            <td><?php echo $aa->functionality ?></td>
                                            <td><?php echo $aa->data_handling ?></td>
                                            <td><?php echo $aa->sustainability ?></td>
