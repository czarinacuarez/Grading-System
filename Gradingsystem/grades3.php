    ...
    <section class="dashboard">
        ...
        <div class="container">
                    <div class="card">
                        ...
                        <?php
                                    $ret = "SELECT * FROM  groups3 WHERE groups3.group3_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {

                                        $sql = "SELECT AVG(grades3.total_grade) AS average_value, groups.group_name AS group_name
                                        FROM groups3
                                        INNER JOIN grades3 ON grades3.group_id = groups3.group3_id
                                        INNER JOIN groups ON groups.group_id = groups3.group_id
                                        WHERE groups3.group3_id = '$id'";  
                        ...
                        <!-- Chart wrapper -->
                            ...
                            <?php
                            $sql = "SELECT CONCAT(user.firstname, ' ', user.lastname) AS name, grades3.total_grade FROM grades3 INNER JOIN user ON user.id = grades3.user_id
                            WHERE grades3.group_id = '$id'";
                            $result = mysqli_query($conn, $sql);
                            ...
                            $sql_count = "SELECT COUNT(*) as total_count FROM grades3  INNER JOIN user ON user.id = grades3.user_id
                            WHERE grades3.group_id = '$id'";
        ...
        <div class = "margin">
            ...
            <div class="table-responsive">
                            ...
                            <table class="table align-items-center table-flush">
                                ...
                                <tbody>
                                  <?php
                                    $ret = "SELECT * FROM  grades3 INNER JOIN user ON grades3.user_id = user.id WHERE group_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {
