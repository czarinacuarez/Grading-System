    ...
    <section class="dashboard">
        ...
        <div class="container">
                    <div class="card">
                        ...
                        <?php
                                    $ret = "SELECT * FROM  groups1 WHERE groups1.group1_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {

                                        $sql = "SELECT AVG(grades1.total_grade) AS average_value, groups.group_name AS group_name
                                        FROM groups1
                                        INNER JOIN grades1 ON grades1.group_id = groups1.group1_id
                                        INNER JOIN groups ON groups.group_id = groups1.group_id
                                        WHERE groups1.group1_id = '$id'";  
                        ...
                        <!-- Chart wrapper -->
                            ...
                            <?php
                            $sql = "SELECT CONCAT(user.firstname, ' ', user.lastname) AS name, grades1.total_grade FROM grades1 INNER JOIN user ON user.id = grades1.user_id
                            WHERE grades1.group_id = '$id'";
                            $result = mysqli_query($conn, $sql);
                            ...
                            $sql_count = "SELECT COUNT(*) as total_count FROM grades1  INNER JOIN user ON user.id = grades1.user_id
                            WHERE grades1.group_id = '$id'";
                            ...
                            <table class="table align-items-center table-flush">
                                ...
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th scope="col">Cost Effectiveness</th>
                                ...
                                <tbody>
                                  <?php
                                    $ret = "SELECT * FROM  grades1 INNER JOIN user ON grades1.user_id = user.id WHERE group_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {
                                    ?>
                                        <tr>
                                            ...
                                            <td><?php echo $aa->robustness ?></td>
                                            <td><?php echo $aa->scalability ?></td>
                                            <td><?php echo $aa->user_experience ?></td>
                                            <td><?php echo $aa->cost_effectiveness ?></td>
