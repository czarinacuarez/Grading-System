        ...
        <div class="container">
                    <div class="card">
                        ...
                        <!-- Chart wrapper -->
                            ...
                            <?php
                            $sql = "SELECT CONCAT(user.firstname, ' ', user.lastname) AS name, grades4.percent FROM grades4 INNER JOIN user ON user.id = grades4.user_id
                            WHERE grades4.group_id = '$id'";
                            $result = mysqli_query($conn, $sql);
                            ...
                            $sql_count = "SELECT COUNT(*) as total_count FROM grades4  INNER JOIN user ON user.id = grades4.user_id
                            WHERE grades4.group_id = '$id'";
        ...
        <div class = "margin">
            ...
            <div class="table-responsive">
                            ...
                            <table class="table align-items-center table-flush">
                                ...
                                <tbody>
                                  <?php
                                    $ret = "SELECT * FROM  grades4 INNER JOIN user ON grades4.user_id = user.id WHERE group_id = '$id'";
                                    $stmt = $conn->prepare($ret);
                                    $stmt->execute();
                                    $res = $stmt->get_result();
                                    while ($aa = $res->fetch_object()) {
                                    ?>
                                        <tr>
                                            ...
                                            <td><?php echo $aa->visual_design ?></td>
                                            <td><?php echo $aa->user_experience ?></td>
                                            <td><?php echo $aa->functionality ?></td>
                                            <td><?php echo $aa->accessibility ?></td>
                                            <td><?php echo $aa->documentation ?></td>
                                            <td><?php echo $aa->overall_quality ?></td>
                                            <td><?php echo $aa->total_grade ?></td>
